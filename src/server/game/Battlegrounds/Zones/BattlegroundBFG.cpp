...
void BattlegroundBFG::FillInitialWorldStates(WorldPackets::WorldState::InitWorldStates& packet)
{
    ...
    for (uint8 node = 0; node < GILNEAS_BG_DYNAMIC_NODES_COUNT; ++node)
        packet.Worldstates.emplace_back(uint32(GILNEAS_BG_OP_NODEICONS[node]), int32((m_Nodes[node] == 0) ? 1 : 0));

    // Node occupied states
    for (uint8 node = 0; node < GILNEAS_BG_DYNAMIC_NODES_COUNT; ++node)
        for (uint8 i = 1; i < GILNEAS_BG_DYNAMIC_NODES_COUNT; ++i)
            packet.Worldstates.emplace_back(uint32(GILNEAS_BG_OP_NODESTATES[node] + plusArray[i]), int32((m_Nodes[node] == i) ? 1 : 0));
    ...
    packet.Worldstates.emplace_back(uint32(GILNEAS_BG_OP_OCCUPIED_BASES_ALLY), int32(ally));
    packet.Worldstates.emplace_back(uint32(GILNEAS_BG_OP_OCCUPIED_BASES_HORDE), int32(horde));

    // Team scores
    packet.Worldstates.emplace_back(uint32(GILNEAS_BG_OP_RESOURCES_MAX), int32(GILNEAS_BG_MAX_TEAM_SCORE));
    packet.Worldstates.emplace_back(uint32(GILNEAS_BG_OP_RESOURCES_WARNING), int32(GILNEAS_BG_WARNING_NEAR_VICTORY_SCORE));
    packet.Worldstates.emplace_back(uint32(GILNEAS_BG_OP_RESOURCES_ALLY), int32(m_TeamScores[TEAM_ALLIANCE]));
    packet.Worldstates.emplace_back(uint32(GILNEAS_BG_OP_RESOURCES_HORDE), int32(m_TeamScores[TEAM_HORDE]));
...
void BattlegroundBFG::_NodeOccupied(uint8 node, Team team)
{
    ...
    if (!trigger)
        trigger = AddCreature(WORLD_TRIGGER, node+5, GILNEAS_BG_NodePositions[node], GetTeamIndexByTeamId(team));
...
bool BattlegroundBFG::SetupBattleground()
{
    ...
    {
        TC_LOG_ERROR("sql.sql", "BatteGroundBGF: Failed to spawn door object Battleground not created!");
        return false;
    }
